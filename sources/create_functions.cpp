...
void RayTracerApp::createTextureImage() {
  ...
  stbi_uc *pixels = stbi_load(std::string(TEXTURE_PATH).c_str(), &texWidth,
...
VkImageView RayTracerApp::createImageView(VkImage image, VkFormat format,
...
void RayTracerApp::createCommandBuffers() {
  ...
  for (size_t i = 0; i < commandBuffers.size(); ++i) {
    ...
    // 3. firstVertex
    // 4. firstInstance
    vkCmdDrawIndexed(commandBuffers[i], static_cast<uint32_t>(tutorial_model.indices.size()),
                     1, 0, 0, 0);
...
void RayTracerApp::createBuffer(
  ...
  if (vkAllocateMemory(device, &allocInfo, nullptr, &bufferMemory) !=
      VK_SUCCESS) {
    throw std::runtime_error("failed to callocate vertex buffer"
                             "memory!");
  }
