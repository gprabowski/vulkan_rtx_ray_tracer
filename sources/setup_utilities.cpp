...
void RayTracerApp::pickPhysicalDevice()
{
...
bool RayTracerApp::isDeviceSuitable(VkPhysicalDevice device)
{
    ...
    bool rtSupported = checkRTExtensionSupport(device);
    ...
    return indices.isComplete() && extensionsSupported && rtSupported && swapChainAdequate &&
           supportedFeatures.samplerAnisotropy;
}

bool RayTracerApp::checkDeviceExtensionSupport(VkPhysicalDevice device)
{
...
bool RayTracerApp::checkRTExtensionSupport(VkPhysicalDevice device)
{
    uint32_t extensionCount;
    vkEnumerateDeviceExtensionProperties(device, nullptr, &extensionCount, nullptr);
    std::vector<VkExtensionProperties> availableExtensions(extensionCount);
    ...
    std::set<std::string> requiredExtensions(rtExtensions.begin(), rtExtensions.end());
